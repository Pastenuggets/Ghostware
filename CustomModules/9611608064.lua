...
runcode(function()
	...
	br = {
		...
		["CheckWhitelisted"] = function(plr, ownercheck)
			...
			local localstr = WhitelistFunctions:Hash(lplr.Name..lplr.UserId)
		...
		["IsVapePrivateIngame"] = function()
			for i,v in pairs(players:GetChildren()) do 
				local plrstr = WhitelistFunctions:Hash(v.Name..v.UserId)
				if WhitelistFunctions:CheckPlayerType(v) ~= "DEFAULT" or WhitelistFunctions.WhitelistTable.chattags[plrstr] then 
					return true
				end
			end
			return false
