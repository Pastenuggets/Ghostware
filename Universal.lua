...
runFunction(function()
	...
	local GamingChairTrails = {Enabled = false}
	local GamingChairSound = {Value = ""}
	local GamingChairSound2 = {Value = ""}
	local chair
	local chairanim
	...
	GamingChair = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then 
				local successfulcustom = false
				if GamingChairSound.Value ~= "" and GamingChairSound2.Value ~= "" then
					 successfulcustom= true
					if (not isfile(GamingChairSound.Value)) and (not isfile(GamingChairSound2.Value)) then 
						warningNotification("GamingChair", "Missing file", 5)
						successfulcustom = false
					end
				end
				...
				movingsound.SoundId = (successfulcustom and getcustomasset(GamingChairSound.Value) or downloadVapeAsset("vape/assets/ChairRolling.mp3"))
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new("Sound")
				flyingsound.SoundId = (successfulcustom and getcustomasset(GamingChairSound.Value) or downloadVapeAsset("vape/assets/ChairFlying.mp3"))
				...
				task.spawn(function()
					repeat
						...
						if entityLibrary.isAlive and entityLibrary.character.Humanoid.Health > 0 then
							...
							if GamingChairTrails.Enabled then
	...
	GamingChairSound = GamingChair.CreateTextBox({
		Name = "Moving Sound",
		TempText = "File (link)",
		FocusLost = function(enter) 
			if enter then 
	...
	GamingChairSound2 = GamingChair.CreateTextBox({
		Name = "Flying Sound",
		TempText = "File (link)",
		FocusLost = function(enter) 
			if enter then 
	...
	GamingChairTrails = GamingChair.CreateToggle({
		Name = "Trails",
		Default = true,
		Function = function()
			if GamingChair.Enabled then
